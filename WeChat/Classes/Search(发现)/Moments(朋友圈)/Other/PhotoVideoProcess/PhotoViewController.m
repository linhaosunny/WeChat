...
- (void)viewDidLoad {
    ...
    PhotoPickerController *imagePickerVc = (PhotoPickerController *)self.navigationController;
    [[ImageManager manager] getAssetsFromFetchResult:_model.result allowPickingVideo:imagePickerVc.allowPickingVideo completion:^(NSArray<AssetModel *> *models) {
...
- (void)okButtonClick {
   ...
   // [imagePickerVc showProgressHUD];
    ...
    [ImageManager manager].shouldFixOrigentation = YES;
    for (NSInteger i = 0; i < _selectedPhotoArr.count; i++) {
        AssetModel *model = _selectedPhotoArr[i];
        [[ImageManager manager] getPhotoWithAsset:model.asset completion:^(UIImage *photo, NSDictionary *info, BOOL isDegraded) {
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    cell.didSelectPhotoBlock = ^(BOOL isSelected) {
        ...
        if (isSelected) {
            ...
            PhotoPickerController *imagePickerVc = (PhotoPickerController *)weakSelf.navigationController;
...
- (void)getSelectedPhotoBytes {
    [[ImageManager manager] getPhotosBytesWithArray:_selectedPhotoArr completion:^(NSString *totalBytes) {
...
- (void)resetCachedAssets {
    [[ImageManager manager].cachingImageManager stopCachingImagesForAllAssets];
...
- (void)updateCachedAssets {
    ...
    if (delta > CGRectGetHeight(_collectionView.bounds) / 3.0f) {
        ...
        [[ImageManager manager].cachingImageManager startCachingImagesForAssets:assetsToStartCaching
                                                                       targetSize:AssetGridThumbnailSize
                                                                      contentMode:PHImageContentModeAspectFill
                                                                          options:nil];
        [[ImageManager manager].cachingImageManager stopCachingImagesForAssets:assetsToStopCaching
                                                                      targetSize:AssetGridThumbnailSize
                                                                     contentMode:PHImageContentModeAspectFill
                                                                         options:nil];
...
- (NSArray *)assetsAtIndexPaths:(NSArray *)indexPaths {
    ...
    for (NSIndexPath *indexPath in indexPaths) {
        AssetModel *model = _photoArr[indexPath.item];
