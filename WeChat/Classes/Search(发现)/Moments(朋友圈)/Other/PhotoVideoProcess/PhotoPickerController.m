...
- (void)viewDidLoad {
    ...
    [ImageManager manager].shouldFixOrigentation = NO;
    ...
    if (iOS9Later) {
        barItem = [UIBarButtonItem appearanceWhenContainedInInstancesOfClasses:@[[PhotoPickerController class]]];
    } else {
        barItem = [UIBarButtonItem appearanceWhenContainedIn:[PhotoPickerController class], nil];
    }
...
- (instancetype)initWithMaxImagesCount:(NSInteger)maxImagesCount delegate:(id<PhotoPickerControllerDelegate>)delegate {
    ...
    if (self) {
        ...
        if (![[ImageManager manager] authorizationStatusAuthorized]) {
...
- (void)pushToPhotoPickerVc {
    ...
    if (_pushToPhotoPickerVc) {
        PhotoViewController *photoPickerVc = [[PhotoViewController alloc] init];
        [[ImageManager manager] getCameraRollAlbum:self.allowPickingVideo completion:^(AlbumModel *model) {
...
@interface AlbumPickerController ()<UITableViewDataSource,UITableViewDelegate> {
...
- (void)configTableView {
    PhotoPickerController *imagePickerVc = (PhotoPickerController *)self.navigationController;
    [[ImageManager manager] getAllAlbums:imagePickerVc.allowPickingVideo completion:^(NSArray<AlbumModel *> *models) {
        ...
        [_tableView registerNib:[UINib nibWithNibName:@"AlbumCell" bundle:nil] forCellReuseIdentifier:@"AlbumCell"];
